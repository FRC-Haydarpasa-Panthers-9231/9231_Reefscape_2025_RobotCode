...
public class RobotContainer {
    ...
    /**
     * Kontrolcüler. 1. porttaki driver'ın kontrolcüsü 2.porttaki operator'ün
     * kontrolcüsü 3.porttaki
     * robotu test ederken extra özelliklere ihtiyaç duydugumuz ve tuş kalmadıgı ya
     * da diger
     * kontrolcülerin butonlarını degiştirmek istemedigimiz için kullandıgımız
     * kontrolcü
     */
    ...
    private final CommandXboxController debugController = new CommandXboxController(
            Constants.OperatorConstants.kPracticeControllerPort);

    // Eger kontrolcüler baglı degilse dashboardda uyarı çıkarır
    private final Alert driverDisconnected = new Alert("Driver kontrolcusunun baglantisi yok!! (port 0).",
            AlertType.kWarning);
    private final Alert operatorDisconnected = new Alert("Operator kontrolcusunun baglantisi yok! (port 1).",
            AlertType.kWarning);

    private final Alert debugControllerDisconnected = new Alert("Debug kontrolcusunun baglantisi yok! (port 2).",
            AlertType.kInfo);
    ...
    private final LoggedNetworkNumber endgameAlert1 = new LoggedNetworkNumber("/SmartDashboard/Endgame Alert #1", 30.0);
    private final LoggedNetworkNumber endgameAlert2 = new LoggedNetworkNumber("/SmartDashboard/Endgame Alert #2", 15.0);
    ...
    public RobotContainer() {
        ...
        Command startPoint = Commands.runOnce(
                () -> {
                    try {
                        drive.setPose(
        ...
        /**
         * Maç bitimi uyarıları, belirtilen saniyelere geldiginde ledleri endgame alert
         * moduna alır ve
    ...
    private void debugControllerBindings() {
        ...
        debugController.rightBumper().onTrue(new IntakingCoral(elevatorRoller));
        debugController
                .leftBumper()
