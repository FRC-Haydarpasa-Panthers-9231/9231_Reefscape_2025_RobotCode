...
public class SUB_LED extends SubsystemBase {
  ...
  private final int length; // LED şeridinin uzunlugu
  public boolean endgameAlert = false;
  public boolean lowBatteryAlert = false;

  // LED efektlerini temsil eden durumlar
  public enum LEDState {
    RAINBOW, // Gökkuşagı efekti
  ...
  private int hue; // Gökkuşagı efekti için kullanılan renk tonu

  /**
   * AddressableLedStrip yapıcısı, LED şeridini başlatır.
   *
   * @param port LED şeridinin baglı oldugu PWM portu
   * @param length LED şeridinin uzunlugu
   */
  public SUB_LED(int port, int length) {
    ...
    ledStrip.setLength(ledBuffer.getLength()); // LED tamponunun uzunlugunu ayarlar
    ledStrip.setData(ledBuffer); // Tampon verilerini LED şeridine uygular
    state = LEDState.RAINBOW; // Varsayılan LED durumu olarak gökkuşagı efekti belirlenir
    hue = 0; // Gökkuşagı efekti için başlangıç tonu
    ledStrip.start(); // LED şeridini çalıştırır
  }

  /**
   * periodic() metodu, WPILib tarafından her döngüde çagrılır. LED efektleri bu metot içinde
   * kontrol edilir.
   */
  @Override
  public void periodic() {
    ...
    if (DriverStation.isDisabled()) {
      if (lowBatteryAlert) {
        ...
        // Robot devre dışıysa, gökkuşagı efekti göster
        rainbow();
      }
  ...
  /** Gökkuşagı efekti uygular. */
  private void rainbow() {
    for (var i = 0; i < length; i++) {
      final var hue = (this.hue + (i * 180 / length)) % 180;
      ledBuffer.setHSV(i, hue, 255, 128);
    }
    this.hue += 3; // Renk tonunu degiştirerek gökkuşagı animasyonu oluştur
    this.hue %= 180;
  }
  ...
  /**
   * LED'lere sırayla iki farklı renk uygular (örnegin, mavi ve kırmızı).
   *
